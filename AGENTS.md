...
- 群聊短时上下文续话：同一群、同一用户在小月成功直达回复后进入 90 秒窗口，最多分析 10 条后续消息，连续 5 条非续聊自动静默关闭；窗口内允许语义分析但不占普通群发送配额，旧的 90 秒/1 条 follow-up 仍兼容；服务端显式澄清提示支持书名、链接、简介等补充请求。
...
- 受控私聊/群聊联网检索已实现并部署：书名/作品查询规则、HTTP(S) 来源硬门槛、群真实直达门禁、按群冷却/小时限额/单次预算、失败释放预留、作品资料来源过滤与 LLM 故障来源兜底均已纳入；本轮补充小说快速引擎、候选抓页→正文复筛、结构化字段清洗与旧检索签名兼容，部署提交为 `a9e0918`。
- 小说研究来源质量门禁已完成：起点等一手作品页优先，百科/结构化资料补充，章节聚合/转载站不进入事实资料块；回答只能据本轮摘录，来源正文不足或仅有低质来源时固定降级；真实只读验收已验证作者、主角、类型、章节、字数和设定线索均来自目标作品正文。
...
- 2026-09-20 联网检索 P0→P3 代码优化已完成：补齐后端状态/缓存/熔断与脱敏 trace、DNS 公网校验与查询清洗、回环代理回退、lxml 正文抽取、失败候选补抓、通用内容相关性/Jaccard 质量门禁、证据长度/正文类型标记、空证据固定降级和长文压缩；配置均支持环境变量，质量字段仅保留在后台结果/trace。

### 未完成/明确限制

- QQ 文本真机链路已验收；当前群聊真实 At/Reply/前缀成功后开启 90 秒、最多 10 条的上下文分析窗口，连续 5 条非续聊自动关闭；窗口内不占普通群发送配额，旧的 90 秒/1 条 follow-up 仍兼容，文字前缀按用户选择保持关闭。
...
- 生产小说只读验收已完成：快速引擎查询在约 1–4 秒内命中，3 个 QQ 阅读相关官方页面正文抓取成功；来源顺序、同名异作摘要排除、正文字段边界和摘要降级均已验证。
- QQ 网关入站/出站与服务端链路稳定；上游 `chat/completions` 仍可能返回 503，因此生产完整 QQ 生成回复验收继续暂缓；本轮只读证据验收不依赖该上游。
...
### 2026-09-20 — QQ 小说回复兜底与窗口观点续问修复
- 代码/配置：小说检索有自然总结时保留正文，不再因局部“无法核实”替换成来源链接；LLM 失败时改为有限资料摘录和诚实降级，默认隐藏原始 URL，仅明确索要来源时展示；提示词同步要求自然概括。活动群窗口内的“你怎么看/这本书怎么样/值得看吗”仅在同一用户最近明确书名、未换题时恢复作品主题并构造小说检索计划。
- 验证：机器人相关定向回归 108 passed；QQ 网关回归 28 passed；服务端全量 1815 passed；Ruff、compileall、git diff --check 全通过；提交前暂存区脱敏扫描 0 命中。
- 提交：代码 `b1bb494` 已提交并推送 GitHub；本次运行状态补记随本次状态提交。
- 运行状态：部署仓已同步 `b1bb494`；`personal-assistant`、`personal-qq-gateway` 均 active；服务端 `/api/health`、`/api/ready` 与网关 `/health` 均正常。
- 未完成：生产搜索/生成上游仍阻塞《没钱修什么仙》只读验收；恢复后重跑来源顺序与正文证据验收；无新增代码阻塞。

### 2026-09-20 — 小说联网证据结构化回复与生产只读验收
- 代码/配置：小说检索接入快速引擎配置；改为候选搜索→官方页面抓取→正文复筛三阶段；过滤同名异作和章节聚合来源；清洗作者、类型、主角、章节、字数、简介与设定线索；旧检索调用增加可选参数签名兼容；补充配置模板与回归测试。
- 验证：小说/群聊定向回归 181 passed；服务端全量 1824 passed；Ruff、compileall、git diff --check 通过；真实只读验收命中 3 个 QQ 阅读相关官方页面，正文抓取 3/3 成功，作者、主角、类型、章节、字数和法力贷设定线索均来自目标作品正文；部署后 `/api/health`、`/api/ready`、QQ 网关 `/health` 均正常，服务 active。
