...
> 状态：六课带教全部完成 · 进阶课 9/10（第 6 课 CI / 第 7 课仪表盘待做）· **467 个测试全绿** · QQ 接入已上线 · 全自动运行中
...
| 项 | 状态 |
|----|------|
| 单元/集成测试 | **467 个**（`cd server && .venv\Scripts\python -m pytest tests/ -q`，覆盖鉴权/执行器安全与绕过变形/命令误吞/确认层/启动器劫持/记忆/隐私脱敏/采集器可靠性/备份可恢复性/健身/情绪/自我状态/主动开口/token 记账/搜索/小说/QQ 推送等） |
