...
> 状态：六课带教全部完成 · 进阶课 9/10（第 6 课 CI / 第 7 课仪表盘待做）· **459 个测试全绿** · QQ 接入已上线 · 全自动运行中
...
| **拟人** | 自我状态（熟络度/久别/刚被纠正）、隔日情绪跟进（记得你昨天不顺）、不确定就直说、主动开口通道（默认关闭，每日 1 条 + 夜间静默 + 无回应降频） |
...
| **运维** | 5 个定时任务、每日 03:00 热备份（校验 + gzip，3 份日备 + 4 份周备）、行为事件按类留存期淘汰、采集通道停滞告警、开机自启 + 崩溃自愈、黑匣子日志 |
...
| 项 | 状态 |
|----|------|
| 单元/集成测试 | **459 个**（`cd server && .venv\Scripts\python -m pytest tests/ -q`，覆盖鉴权/执行器安全与绕过变形/命令误吞/确认层/启动器劫持/记忆/隐私脱敏/采集器可靠性/备份可恢复性/健身/情绪/自我状态/主动开口/搜索/小说/QQ 推送等） |
| 测试隔离 | conftest 两层护栏：默认库挪出生产库 + `connect()` 上的 time-of-use 拦截器（测试期连生产库直接 RuntimeError）。曾因 14 个测试文件的隔离失效清空过真实库，见 LESSONS 第 14 课 |
