...
# 第二轮评审记录（"Executive Summary" 报告）

> 来源：外部 AI 的 Executive Summary 报告。
> 结论先行：**该报告分析的不是本项目**——它引用的 `app.py`、`app_whatsapp.py`、
> `db/checkpoints.sqlite`、`TelegramChannel`、`personal_assistant.invoke` 等文件/符号
> 在本仓库中均不存在（已 grep 验证）。系 AI 张冠李戴（幻觉或喂错代码）。
> 但其 8 条建议的通用原则中 4 条可映射到本项目真实代码，已采纳（v0.3）。

## 逐条判定

| # | 报告建议 | 判定 | 说明 |
|---|----------|------|------|
| ① | time.sleep 阻塞主循环 | ❌ 不适用 | 项目无 time.sleep；采集循环均为 asyncio.sleep + to_thread |
| ② | check_same_thread=False 共享连接风险 | ❌ 不适用 | 项目每次操作新建连接（无共享连接）；WAL+busy_timeout 已开。连接池化记路线图 |
| ③ | 主循环无异常处理 | ❌ 不适用 | 三通道均有 try/except + 健康上报（v0.1/v0.2 已实现） |
| ④ | to_thread 线程切换开销 | ❌ 不适用 | 同步 IO（复制文件/git 子进程）用 to_thread 是正确用法，开销可忽略 |
| ⑤ | 数据库索引缺失 | ✅ **采纳（v0.3）** | 已加 idx_memories_ts / idx_facts_updated / idx_worklog_created |
| ⑥ | 依赖锁定 + 漏洞审计 | ✅ **采纳（v0.3）** | requirements 全部锁定精确版本（实测通过）；pip-audit 建议纳入发布流程 |
| ⑦ | 测试覆盖不足 | ✅ **采纳（v0.3）** | 测试 3 → 13：新增记忆去重/检索兜底/注入格式/隐私脱敏 10 例 |
| ⑧ | print → logging | ✅ **采纳（v0.3）** | server/collector 全部改用 logging（统一格式，模块名标识） |

## 经验沉淀

1. **AI 报告必须先验证"它评的是不是你的项目"**：本轮报告文件引用全部对不上，
   若不验证直接采纳，会把 Telegram 机器人的问题硬套到本项目。
2. "无需修改项"与"建议项"来源混乱（前半像别的项目、后半像本项目），
   是 AI 拼接幻觉的典型特征：**引用具体文件:行号的建议才可信**。
3. 即便如此，通用原则（索引/锁版本/测试/日志）依然值得逐条映射检查——
   这也是 AI 评审的残余价值所在。
...
> 来源：外部建议的"个人助手功能扩展清单"（Lesson 2.0/Goal/unresolved 等）。
> 筛选标准：个人项目、单进程、SQLite、复杂度换收益比优先。

## 逐项判定

| 建议 | 判定 | 理由 |
|------|------|------|
| Lesson 2.0 结构化反思 | 🔶 暂缓 | 现有 lessons（原文+上下文+注入）够个人量级；教训>50 条才需结构化 |
| **Goal 系统** | ✅ 采纳 | 身份定义"项目信息/阶段目标"的落地；做 |
| **unresolved 问题追踪** | ✅ 采纳 | 聊天中断是真实痛点；做 |
| Decision Memory | 🔶 暂缓 | facts 三元组可近似表达 |
| Hypothesis→Validation | 🔶 暂缓 | 评测脚本（第 10 课）已是该模式 |
| Memory usefulness feedback | 🔶 暂缓 | importance 召回加成已是隐式反馈 |
| Opportunity Detector | ❌ 重复 | 关切追踪 stale 提醒已实现 |
| Concern 优先级模型 | 🔶 暂缓 | mention_count+时间排序已够 |
| 简化关系图谱 | 🔶 暂缓 | facts 三元组已是雏形 |
| 多 Agent / Neo4j / LangGraph / 复杂人格 | ❌ 同意不推荐 | 个人项目过度设计 |

**采纳 2 项**：Goal 系统 + unresolved 追踪（共用"检测→存储→注入"模式，半天工作量）。

## 经验沉淀

外部清单常按"企业级标准"给 P0——**要拿自己的约束重新定价**：
13 项砍到 2 项不是偷懒，是"复杂度换收益比"的活学活用。
