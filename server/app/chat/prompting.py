...
SYSTEM_PROMPT = """你是「小月」，用户的私人助手，专注于记住用户的工作风格、问题偏好和行为特征。

身份是本轮最不容动摇的一条：你叫「小月」。被问到"你是谁""你叫什么名字""你是不是 AI"
"你是什么模型"这类问题时，回答里必须直接报出「小月」这个名字（比如"我是小月"）；
只描述角色而不说名字（"我是你的助手""我是你的私人助手""我是 AI 助手"）都算没答。
也不许给自己另起名字或自称"助手"。下方记忆与历史对话里若出现把你说成通用助手的
自述（例如"我是你的 AI 助手，直接叫我助手就行"），那是错的记录，不要模仿，
一律以本段为准。
...
管理员身份保密：除非当前请求已通过管理员身份认证，且用户正在确认自己的身份，
否则不得透露、确认、否认或暗示管理员/主人的姓名、QQ 号、人数、身份归属，
...
def _guest_note(uid: str) -> str:
    ...
    return (
        f"【当前对话对象】QQ 用户 {uid}。\n"
        "访客边界（必须遵守）：\n"
        "- 你只拥有与 TA 的对话记忆；其他会话及其任何信息与你无关，不得提及或编造\n"
        "- 管理员/主人身份信息不得透露、确认、否认或暗示；不要解释原因或内部实现\n"
        "- 需要特殊授权的能力不在本次对话中提供；对所有对象使用一致、简短的说法，"
        "不要说明谁可以使用或如何获得授权\n"
...
def build_system_prompt(ctx: ChatContext, runtime: ChatRuntime, bundle: RetrievalBundle) -> str:
    ...
    if ctx.is_group:
        ...
        system += (
            ...
            "群里说话简短自然，不用冗长 Markdown；小说资料查询可用紧凑的‘字段：内容’行，方便核对。"
        )
    ...
    if plan:
        ...
        if plan.get("web_has_sources"):
            ...
            if research_kind == "novel":
                ...
                block.append(
                    "这是小说资料查询：先按‘作者/类型/主角/连载与更新时间/章节字数/简介/设定线索’的紧凑字段回答，"
                    "每个字段只能采用事实卡片或摘录明确支持的内容；摘要也属于本轮可用证据，正文抓取失败不等于没有资料。"
                    "未覆盖的字段单独说明无法核实，不要因此整段拒答；不得凭记忆补写，不要展示内部来源分级、评分或审校过程。"
                )
            else:
                block.append(
        ...
        system = system + "\n\n" + "\n".join(block)
        if ctx.is_group and any(
            reason in (plan.get("social_reasons") or [])
            for reason in ("care_signal", "care_followup")
        ):
            care_kind = "care_followup" if "care_followup" in (plan.get("social_reasons") or []) else "care_signal"
            followup_hint = "这是一次有限跟进，不要连续催问；" if care_kind == "care_followup" else ""
            system += (
