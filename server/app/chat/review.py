...
_NOVEL_SOURCE_REFUSAL_RE = re.compile(
    r"(?:没有|未|无法|不能|没法).{0,28}(?:查到|找到|核实|确认|可靠来源|可靠资料).{0,36}(?:作者|简介|设定|剧情|作品|资料)?"
)


def is_novel_source_refusal(text: str) -> bool:
    """识别把已有小说资料错误改写成整体拒答的候选文本。"""
    return bool(_NOVEL_SOURCE_REFUSAL_RE.search(str(text or "")))
...
时间、排名等具体断言写成事实，应改成承认无法核实或请用户提供链接/原文。小说若已有本轮搜索摘要或作品页来源，
摘要本身就是有限证据；正文抓取失败只限制未覆盖字段，不得把已有摘要整体改写成“没有查到”。
