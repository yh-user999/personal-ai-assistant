...
_KNOWN_OUTLETS: tuple[str, ...] = (
    ...
    "北京日报", "广州日报", "成都商报", "联合早报",
    # 地方媒体与其视频/新媒体品牌（这些常以自身名义首发、被门户转载）
    "潇湘晨报", "晨视频", "大皖新闻", "大风新闻", "新黄河",
    "潮新闻", "齐鲁晚报", "大河报", "华商报",
    "南方人物周刊", "经济观察报", "财经杂志", "蓝鲸新闻",
    "中国新闻网", "中国网", "中国日报", "科技日报", "工人日报",
    # 境外与专业媒体
    "路透社", "彭博", "美联社", "法新社", "BBC", "纽约时报",
    "金融时报", "华尔街日报", "南华早报", "共同社", "韩联社",
)
...
_ORIGIN_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    # 「据晨视频9月8日报道」——媒体名与"报道"之间常夹日期
    re.compile(
        r"据\s*([\u4e00-\u9fa5A-Za-z]{2,12}?)\s*"
        r"(?:\d{4}年)?(?:\d{1,2}月\d{1,2}日|\d{1,2}日)?\s*"
        r"(?:等媒体)?(?:报道|消息|称|披露)"
    ),
    # 人民日报客户端发表评论文章 / 中新社发布
    ...
    for pattern in _ORIGIN_PATTERNS:
        ...
        if match:
            ...
            if len(candidate) >= 2 and candidate not in {"记者", "报道", "消息", "近日"}:
                return candidate
    # 兜底：查"署名位"的已知媒体名。
    # 关键是**只认署名位，不认正文提及**——实测踩坑：放宽成"出现即算"后，
    # 腾讯自采稿因标题带"人民日报发声"、联合早报稿因引述人民日报评论，
    # 双双被归到人民日报名下，4 条转载被误算成 6 条，还抹掉了两个真正
    # 独立的信源。核查本身反倒制造了虚假的集中。
    return _origin_from_signature_position(text)


# 署名位允许的后缀。刻意**不含**"发声/回应"——"人民日报发声"通常出现在
# 别家的标题里（A 媒体转述 B 媒体表态），那是提及而不是署名。
_SIGNATURE_TAIL_RE = re.compile(
    r"^\s*(?:[：:，,、|】\]\)」』]|\s)*(?:评|锐评|评论|社评|报道|发布|刊发|消息|"
    r"记者|独家|调查|客户端|通报|专访)"
)
_LEADING_DECOR_RE = re.compile(r"^[【\[「『“‘\"'\s]+")


def _origin_from_signature_position(text: str) -> str:
    """只在**开头署名位**找已知媒体。

    限定开头是必须的：「被指摸臀4岁男孩已返校；人民日报发声」里人民日报
    出现在中段，那是转述而非署名；早期版本允许出现在前 15 字内，于是这条
    被误判成人民日报原创，把腾讯自采稿并进了官方信源簇。
    """
    head = _LEADING_DECOR_RE.sub("", (text or "").strip())[:60]
    for known in _KNOWN_OUTLETS:
        if not head.startswith(known):
            continue
        tail = head[len(known) :]
        if _SIGNATURE_TAIL_RE.match(tail) or tail.lstrip().startswith(("：", ":")):
            return known
    return ""
