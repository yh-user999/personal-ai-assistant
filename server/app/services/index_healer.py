...
# 二期自动抽取与候选池账本已迁至 services/extraction_candidates.py
# （独立职责：低置信实体候选的登记/确认/废弃 + 每日抽取预算）。
# 以下再导出保持 index_healer.* 既有调用方与测试路径不变。
from app.services.extraction_candidates import (  # noqa: E402, F401
    AUTO_DAILY_LIMIT,
    AUTO_MAX_BLOCKS,
    AUTO_MIN_EVIDENCE,
    _chunk_evidence,
    _reserve_extract_slot,
    _settle_extract_slot,
    auto_extract_task,
    candidate_add,
    candidate_confirm,
    candidate_discard,
